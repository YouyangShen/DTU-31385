function [ route ] = greedy_3d( map, start, end_, length_cost )
    ...
    max_x = length(map(:,1,1,1));
    max_y = length(map(1,:,1,1));
    max_z = length(map(1,1,:,1));
    ...
    while ~(parent_node.position(1) == end_(1) && ...
            parent_node.position(2) == end_(2) && parent_node.position(3) == end_(3))
    ...
    while  ~(parent_node.position(1) == start(1) && ...
             parent_node.position(2) == start(2) && parent_node.position(3)==start(3))
